...
use smart_3d::rrt::{GoalBiasedUniformDistribution, KdTreeNearestNeighbors, RealVectorState, RRT};
...
async fn main() {
    ...
    let mut rrt = RRT::<f32, 2, KdTreeNearestNeighbors<_, 2>>::new(
        start,
        goal,
        goal_tolerance,
    ...
    loop {
        ...
        let nodes: &Vec<smart_3d::rrt::rrt::Node<f32, 2>> = rrt.get_tree();
        for node in nodes {
